...
pub(crate) fn initialize_app(
    ...
    // super-warp-terminal: Use DirectMinimaxClient if MINIMAX_API_KEY is set,
    // otherwise fall back to server-mediated AI client
    let ai_client = if let Some(direct_client) = crate::ai::local_ai_client::DirectMinimaxClient::from_env() {
        log::info!("super-warp-terminal: Using direct MiniMax AI client (standalone mode)");
        direct_client as Arc<dyn crate::server::server_api::ai::AIClient>
    } else {
        log::info!("super-warp-terminal: Using server-mediated AI client");
        server_api_provider.as_ref(ctx).get_ai_client()
    };
